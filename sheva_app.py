...
def save_to_sheet(row_dict):
    try:
        ...
        # קריאת כל הנתונים לעדכון (לא רק headers)
        records = sheet.get_all_records()
        df = pd.DataFrame(records)
        
        # המרת הכל לטקסט למניעת שגיאות סוגי נתונים
...
if uploaded_file:
    ...
    df['ת.ז לקוח'] = df['ת.ז לקוח'].astype(str)
    clients = df.groupby('ת.ז לקוח').agg({'שם לקוח': 'first', 'טלפון סלולרי': 'first', 'פרמיה חודשית': 'sum'}).reset_index()
                ...
                if st.button("שמור שינויים", key=f"b_{cid}"):
                    with st.spinner("שומר לשיטס..."):
                        ...
                        if success:
                            ...
                            # לא עושים rerun אוטומטי כדי שהמשתמש יראה את ההודעה
